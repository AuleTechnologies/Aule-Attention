...
Automatically selects the best available backend in priority order:
1. ROCm FlashAttention (CK) - Peak performance (~150-300 TFLOPS on MI300X)
2. PyTorch SDPA on ROCm - High performance (~50-150 TFLOPS)
3. HIP MFMA (our kernel) - Good performance (~50-100 TFLOPS)
4. HIP scalar - Moderate performance
5. Vulkan - Consumer GPUs (AMD, NVIDIA, Intel, Apple)
6. OpenCL - Works on MI300X via Mesa Rusticl WITHOUT ROCm
7. CPU fallback - Always available

This library works BOTH with and without ROCm:
- WITH ROCm: Uses official FlashAttention for peak performance
- WITHOUT ROCm: Falls back to OpenCL/Vulkan for portability
...
try:
    # Try package-relative import first, then fallback to direct
    try:
        from .vulkan import Aule, AuleError
    except ImportError:
...
try:
    try:
        from .opencl import OpenCLAttention, is_opencl_available
    except ImportError:
...
try:
    try:
        from .hip import HipAttention, is_hip_available
    except ImportError:
...
try:
    try:
        from .hip_mfma import MFMAAttention, is_mfma_available
    except ImportError:
...
Backend = Literal['auto', 'vulkan', 'opencl', 'hip', 'hip_mfma', 'rocm_flash', 'pytorch_rocm', 'cpu']
...
class Attention:
    ...
    def __init__(self, backend: Backend = 'auto'):
        ...
        if env_backend in ('vulkan', 'opencl', 'hip', 'hip_mfma', 'rocm_flash', 'pytorch_rocm', 'cpu'):
            backend = env_backend
    ...
    def _init_auto(self):
        ...
        # 3. HIP MFMA (our kernel) - ~50-100 TFLOPS
        # 4. HIP scalar - Moderate performance
        # 5. Vulkan - Consumer GPUs
        # 6. OpenCL - MI300X via Mesa Rusticl without ROCm
        # 7. CPU fallback

        # Try ROCm FlashAttention first (peak performance)
...
def get_available_backends() -> list:
    ...
    # ROCm backends (highest priority)
    if _rocm_flash_available:
...
def get_backend_info() -> dict:
    ...
    return {
        ...
        'pytorch_rocm': {
            ...
            'description': 'PyTorch SDPA on ROCm',
            'performance': '~50-150 TFLOPS on MI300X',
            'requires': 'ROCm + PyTorch ROCm build',
        },
        'hip_mfma': {
        ...
        'opencl': {
            ...
            'description': 'OpenCL (Mesa Rusticl)',
...
def print_backend_info():
    ...
    for backend in ['rocm_flash', 'pytorch_rocm', 'hip_mfma', 'hip', 'vulkan', 'opencl', 'cpu']:
        data = info[backend]
